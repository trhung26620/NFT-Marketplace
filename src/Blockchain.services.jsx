...
const isWalletConnected = async () => {
    try {
        ...
        console.log("🚀 ~ file: Blockchain.services.jsx:59 ~ isWalletConnected ~ error", error)
...
const mintNFT = async ({ title, description, metadataURI, price }) => {
    try {
        ...
        await contract.methods.payToMint(title, description, metadataURI, price)
...
const buyNFT = async ({ id, cost }) => {
    try {
        ...
        const contract = await getEthereumContract();
        const connectedAccount = getGlobalState('connectedAccount');
        await contract.methods.payToBuy(Number(id))
            .send({
...
export {
    connectWallet, isWalletConnected, mintNFT, getAllNFTs, updateNFT, buyNFT
}
